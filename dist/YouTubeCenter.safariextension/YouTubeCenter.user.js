...
(function(){
  ...
  var ___main_function = function(injected, identifier){
    ...
    ytcenter.thumbnail = (function(){
          ...
          $XMLHTTPRequest({
            ...
            onload: function(r){
              try {
                ...
                item.hq = ytcenter.player.getClosestQuality("highres", ytcenter.parseStream(cfg.args));
                if (isInCache(item)) {
      ...
      function appendQuality(item, hq, errorMessage) {
        var tableQuality = {
              "error": errorMessage,
              "auto": errorMessage,
              "small": "240p",
        ...
        if (ytcenter.settings.videoThumbnailQualityDownloadAt === "hover_thumbnail") {
          ytcenter.utils.addEventListener(item.wrapper, "mouseover", (function(){
            ...
            return function(){
              if (added) return;
              added = true;
              loadVideoConfig(item, function(hq, errorMessage){
                appendQuality(item, hq, errorMessage);
              });
            };
          })(), false);
        } else {
